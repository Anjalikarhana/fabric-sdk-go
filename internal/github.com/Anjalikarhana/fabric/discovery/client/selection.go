...
import (
	...
	"github.com/Anjalikarhana/fabric-sdk-go/internal/github.com/Anjalikarhana/fabric/gossip/protoext"
)
...
//	Compare(Peer, Peer) Priority
	...
	// match func matches location of txn with dc location where peer is located 
	// their relative scores
	match_dC(txn_location, Peer) Priority
}
...
var (
	...
	// PrioritiesByDC selects peers by nearest DC -(change-1)
	PrioritiesByDC = &byDC{}
...
// this func will compare the location with the peers 
func (*byDC) match_dC(txn_location string , p Peer) Priority{
	p_DC :=  p.StateInfoMessage.GetStateInfo().Properties.DC_NAME

	if txn_location == p_DC{
	return 1
    }
	else {
	return -1	
